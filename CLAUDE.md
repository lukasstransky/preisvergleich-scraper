...
**Firestore sync is diff-based and quota-sensitive** — this is the core of the system. `firebase_store.upload_all` → `firestore_sync.sync_products` per supermarket. All stores share one flat `products` collection. The sync state (`{hashes, prices}` per supermarket) is stored **locally on disk** in `sync_state/{supermarket}.json` (gitignored), NOT in Firestore — it holds one entry per product and would breach Firestore's per-document limits (40k index entries / 1 MiB) as a single doc. It's pure sync bookkeeping the app never reads, so a local file (per machine running the sync) is the right home. Override the location with the `SYNC_STATE_DIR` env var (tests do this for isolation via a `conftest.py` autouse fixture).
- Products are MD5-hashed; only new/changed products are written to Firestore and removed ones deleted (compared against stored `hashes`).
